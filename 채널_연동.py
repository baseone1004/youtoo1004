...
def fetch(cfg, channel):
    ...
    try:
        ...
        if info and info.get("videos") is not None:
            videos = [dict(title=v["title"], views=int(v.get("views") or 0), duration=int(v.get("duration") or 0), url=v.get("url", ""))
            ...
            data[channel] = dict(url=url, name=info.get("name") or url, titles=[v["title"] for v in videos], videos=videos,
                                 subs=int(info.get("subs") or 0), fetched=now, error="")
        else:
...
def analysis(cfg, channel):
    ...
    if not url or info.get("url") != url or not videos:
        return dict(ok=False, count=len(info.get("titles") or []), reason="채널 제목을 아직 읽지 않았거나 조회수 정보가 없습니다. [제목 다시 읽기]를 눌러 주세요.")
    ...
    return dict(ok=True, name=info.get("name", ""), subs=info.get("subs", 0), count=len(videos), fetched=info.get("fetched", ""),
                avg=round(avg), median=median, total=sum(views),
                top=ranked[:5], bottom=ranked[-3:][::-1] if len(ranked) > 5 else [],
